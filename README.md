...
A working local prototype of a counterfactual documentary, built from Beaven’s conversations with Copilot/Mirrows. Explore the recorded journey, choose a conversational cutoff, change the model’s context, and write the human side of another branch.

Hosted exploration: **[man-of-la-machina.com](https://man-of-la-machina.com)**. Application code is public; the transcript bundle is loaded server-side from a private Railway bucket. See [hosting and private content releases](docs/HOSTING.md).
...
See [curation instructions](docs/CURATION.md), [implementation decisions and remaining exhibition work](docs/IMPLEMENTATION.md), and [design and verification notes](docs/VERIFICATION.md). Public-mode validation rejects unreviewed/private content. The hosted exploration edition preserves the review flags and is not certified for exhibition.
